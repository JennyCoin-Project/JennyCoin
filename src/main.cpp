...
uint256 hashGenesisBlock("0xcf76a91ab3b964c6f27aa09704ff660fb74ad55ad60db86eff789aec9eb4c228");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // JennyCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "JennyCoin Signed Message:\n";
...
{
    // JennyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 867.5309 * COIN;
		
	if(nHeight >= 1 && nHeight <= 10)  
    {
        nSubsidy = 867530.9 * COIN;
    }
	else if(nHeight >= 11 && nHeight <= 60)
	{
		nSubsidy = 0 * COIN;
	}
	else if(nHeight >= 61 && nHeight <= 100)
	{
		nSubsidy = 10 * COIN;
	}
	else if(nHeight >= 101 && nHeight <= 500)
	{
		nSubsidy = 50 * COIN;
	}
	else if(nHeight >= 501 && nHeight <= 1000)
	{
		nSubsidy = 100 * COIN;
	}
	else if(nHeight > 1000)   
    {
        nSubsidy = 867.5309 * COIN;
    }
	
//		nSubsidy >>= (nHeight / 5000); //halve block reward every 5000 blocks
...
static const int64 nTargetTimespan = 1 * 60; // 1 minute blocks
static const int64 nTargetSpacing = 1 * 60; // 1 minute retareget
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // JennyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (LatestBlockTime < BlockReading->GetBlockTime()) {
            if (BlockReading->nHeight > 450){ // HARD Fork block number
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 1;
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 330) { DiffMode = 1; }
        }
        else {
                if (pindexLast->nHeight+1 >= 450) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // JennyCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // JennyCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "April 04, 2014, JennyCoin will launch";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 867.5309 * COIN;
        ...
        assert(block.hashMerkleRoot == uint256("0x284f41ab4bb3995c7dda0c45a5dff4fcabcd4dd4e94ee0f972ed6d1b39cc27e0"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // JennyCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JennyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JennyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JennyCoinMiner(CWallet *pwallet)
{
    printf("JennyCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("JennyCoin-miner");
    ...
    try { loop {
        ...
        printf("Running JennyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JennyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JennyCoinMiner, pwallet));
}
